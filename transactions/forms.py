...
class SupplierForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['phone'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '10', 'pattern' : '[0-9]{8}', 'title' : 'Numbers only'})
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['gstin'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '15', 'pattern' : '[A-Z0-9]{14}', 'title' : 'GSTIN Format Required'})
    class Meta:
        model = Supplier
...
class SaleForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['gstin'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '15', 'pattern' : '[A-Z0-9]{14}', 'title' : 'RTN Format Required'})
        # self.fields['tax'].widget.attrs.update({'class': 'textinput form-control', 'min': '0', 'max': '100', 'step': '0.01', 'placeholder': 'Impuesto (%)'})
    class Meta:
        model = SaleBill
        fields = ['name', 'phone', 'address', 'email', 'gstin']
        widgets = {
...
class SaleItemForm(forms.ModelForm):
    ...
    class Meta:
        model = SaleItem
        fields = ['stock', 'quantity', 'perprice', 'tax']
